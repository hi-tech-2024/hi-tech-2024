...
Дополнительные материалы и дистрибутивы можно найти на ресурсе `http://files.hitech2024ext.ru`

В рамках задания используется ALD Pro версии 2.3.0
...
3. Комплекты документации доступны на сервере files.hitech2024ext.ru, имитирующем реальную работу сети Интернет.
4. Все сетевые устройства доступны по консоли.
5. На ISP преднастроена запись vpn.hitech2024ext.ru с адресом 203.0.113.100.
6. На ISP преднастроена запись www.hitech2024ext.ru с адресом 203.0.113.100.
...
8. Провайдеронезависимые (PI) адреса и ASN в ЦОДе
...
11. На DNS-сервере (все адреса ISP) настроена зона hitech2024ext.ru
...
1. Настройте fqdn hostname (для ЦОД - *.hitech2024.local, для office и branch офисов - office.hitech2024.local и branch.hitech2024.local) на всех сетевых устройствах
2. Настройте административный доступ ко всем сетевым устройствам.
    ...
    2. На vESR-Office используйте для аутентификации по SSH RADIUS-сервер (DC-A), но предусмотрите локальный вход в случае недоступности сервера.
        - RADIUS сервер должен использовать базу пользователей ALD office.hitech2024.local для аутентификации пользователей.
...
1. Разверните систему управления ЕПП на базе ALD Pro, используйте доменное имя office.hitech2024.local.
...
5. На SRV-A реализуйте почтовый сервер.
    ...
    2. Обеспечьте аутентификацию с использованием доменных учетных данных.
        - Доступ к почте должны получать пользователи office.hitech2024.local
    3. Обеспечьте возможность отправки писем между доменами office и branch. От имени пользователя JohnD на CLI-A отправьте письмо пользователю admin на CLI-B.
    ...
    4. При входе пользователь должен указывать только логин учетной записи и почтовый домен в формате `username@office.hitech2024.local` для домена office и `username@branch.hitech2024.local` для домена branch.
...
1. Разверните контроллер системы управления ЕПП на базе ALD Pro для домена branch.hitech2024.local.
    ...
    3. Сконфигурируйте доверительные отношения с доменом Windows, таким образом, чтобы пользователи домена Windows аутентифицировались на компьютерах с Astra Linux. Импортировать пользователей из домена Windows ЗАПРЕЩЕНО.
2. CLI-B должны аутентифицироваться через ALD Pro.
    1. Для пользователей из доменной группы admin при входе на рабочем столе должны создаваться ярлыки на web интерфейс Zabbix и ALD Pro на DC-B.
...
3. Реализуйте перемещаемые профили только для пользователей домена hitech2024.local на сервере DC-B.
    1. Используйте протокол NFS версии 4.
    2. При входе на другом компьютере пользователь должен получать доступ ко всем своим данным.
    3. Обеспечьте синхронизацию каталогов профилей пользователей между офисами.
4. На DC-B реализуйте почтовый сервер.
    1. Соединение с почтовым сервером должно быть защищено. Клиент не должен получать ошибок в почтовом клиенте.
    2. Обеспечьте аутентификацию с использованием доменных учетных данных.
        - Доступ к почте должны получать пользователи branch.hitech2024.local
    3. Обеспечьте возможность отправки писем между доменами office и branch.
5. При входе пользователь должен указывать только логин учетной записи и почтовый домен в формате `username@office.hitech2024.local` для домена office и `username@branch.hitech2024.local` для домена branch.
        - Почтовый клиент не должен требовать ввода данных почтового сервера и пароля.
            - Клиент не должен запрашивать никаких данных про сервер.
            - Авторизация должна проходить по билету kerberos.
...
1. На серверах ZVIRT-1 и ZVIRT-2 разверните систему виртуализации на ПО zVirt.
    1. Настройте кластер виртуализации в варианте Hosted Engine.
        ...
        - Web интерфейс Hosted engine должен располагаться по адресу `https://engine.hitech2024.local`.
...
`Образ диска виртуальной машины находится на ресурсе files.hitech2024ext.ru`
...
4. На ВМ WEB* установите nginx+php.
    1. Создайте сайт `www.hitech2024ext.ru`. Приложение для сайта находится на ресурсе `files.hitech2024ext.ru`.
...
7. На виртуальной машине WEB1 реализуйте централизованный сбор журналов.
    ...
    2. Настроить запись журналов в СУБД postgres. Журналы необходимо писать в БД syslog.
    3. На CLI-* настроить сбор журнала всех команд, вводимых в терминале только для доменных пользователей. Настроить запись журналов в директорию /opt/logs/hostname.log на сервере WEB2.
        ...
        - Сконфигурируйте web сервер для доступа к файлам журналов на WEB2 по адресу: `https://logs.hitech2024.local`.
        - Настройте пернаправление с http на https.
8. На виртуальной машине WEB2 реализуйте службу мониторинга zabbix в связке с системой визуализации данных grafana.
    ...
    6. Веб интерфейс Zabbix должен быть доступен по адресу: `https://zabbix.hitech2024.local`.
        - Настройте пернаправление с http на https.
    7. Веб интерфейс grafana должен быть доступен по адресу: `https://grafana.hitech2024.local`.
...
9. На виртуальной машине WEB2 разверните корпоративный репозиторий.
    ...
    2. Подключите корпоративный репозиторий по записи `deb https://repo.hitech2024.local hitech2024 hitech2024-apps` на всех виртуальных серверах в ЦОДе и на CLI-*.
...
10. Создайте deb пакет с именем hitech2024-config. Deb пакет при установке должен выполнять следующие настройки:
    ...
    5. Добавьте пакет в корпоротивный репозиторий.
11. На виртуальной машине PROXY разверните cервис GitFlic для хранения исходного кода и работы с ним. (Все необходимые пакеты можно загрузить с портала files.hitech2024ext.ru).
    1. Создайте локального админ пользователя `gitadmin@yandex.ru` с паролем P@ssw0rdSkills.
    2. Настройте доступ по ldap для пользователей домена hitech2024.local.
    3. Создайте проект hitech2024-config.
        - Опубликуте исходный код deb-пакета hitech2024-config.
    4. Веб интерфейс GitFlic должен быть доступен по адресу: `https://git.hitech2024.local`.
        - Настройте пернаправление с http на https.
...
| :-:             | :-:              | :-:                       |
|       DC        |hitech2024\Administrator, Administrator|P@ssw0rd|
|       IIS       |hitech2024\Administrator, Administrator|P@ssw0rd|
|       RDS       |hitech2024\Administrator, Administrator|P@ssw0rd|
|       WinCli    |               User                  |P@ssw0rd|

Ваша компания имеет внутренний домен hitech2024.local. В домене имеются некоторые группы безопасности, нужные по заданию и около 300 пользователей (Пароль для всех пользователей P@ssw0rd1). Они находятся в OU Tasks. Запрещено их переносить в другие OU или добавлять в группы, если это не оговорено в задании.

1. На сервере DC развернуть корпоративный центр сертификации с именем hitech2024-RootCA и сроком жизни корневого сертификата 4 года.
2. Включите рабочую станцию WinCli (OUTTER) в домен hitech2024.local.
3. Включить всем Windows компьютерам домена для браузера Microsoft Edge стартовую страницу `https://portal.hitech.ru`, а пользователям группы hitech2024\RDS users стартовую страницу `https://rds.hitech2024.local`.
4. Создать сайт на IIS с адресом `https://portal.hitech.ru` и наполнением содержащемся в папке C:\portal.
5. Обеспечить доступ к порталу только по протоколу HTTPS. Подписать сертификатом, выданным hitech2024-RootCA. При обращении к сайту не должно отображаться ошибок и предупреждений. При попытке обратиться по адресу `http://portal.hitech.ru` должно следовать подключение по адресу `https://portal.hitech.ru`.
6. Настроить SSO при доступе по адресу `https://portal.hitech.ru` к порталу с учетными записями домена hitech2024.local, входящими в группу безопасности hitech2024\portal. Доступ без ввода пароля должен работать на всех Windows-клиентах, под любым пользователем, входящим в группу безопасности hitech2024\portal, в том числе будущих. Важно! Провека будет осуществляться в Microsoft Edge Browser.
7. Настроить доступ к `https://portal.hitech.ru/admin` через сертификаты, выдаваемые автоматический для группы hitech2024\admin при включении в данную группу, причем доступ в виртуальную папку admin, не означает доступ в корень сайта, т.е. если пользователь есть в группе hitech2024\admin, но его нет в группе hitech2024\portal, то доступа на сайт `https://portal.hitech.ru` быть не должно. Важно! Провека будет осуществляться в Microsoft Edge Browser.
...
12. Настройте доступ к web-интерфейсу RDS по адресу `https://rds.hitech2024.local` без ввода логина и пароля, настройте доступ по HTTPS с использованием сертификата, выданного hitech2024-RootCA. При подключении не должно выдаваться никаких ошибок и предупреждений.
13. Опубликуйте Wordpad для группы “hitech2024\RDS users”. Обеспечьте загрузку файла RDP и запуск сесии без ввода логина и пароля. При подключении и загрузке файла не должно выдаваться никаких ошибок и предупреждений.
14. В домене разверните CEP роль сервера центра сертификации. Настройки доступ к нему по адресу `https://cep.hitech2024.local/<...>`. Авторизация на сервере должна проходить средствами Kerberos. Настройки CEP должны распространяться на все Windows-компьютеры домена. Обеспечьте выдачу сертификата с использованием CEP и шаблона Computer для Windows клиентов из под учетной записи hitech2024\Administrator. При запросе сертификата через оснастку MMC - серитификаты можно выбрать этот CEP.
15. Обеспечьте выдачу сертификата с использованием CEP и шаблона Computer для Linux клиентов. Настройте выдачу сертификата с использованием шаблона Computer на COD-CLI, введеный в домен hitech2024.local. Создайте скрипт для получения сертификата и поместите на рабочий стол под пользователем hitech2024\Administrator.
16. Настройте доверительные отношения между Windows доменом hitech2024.local и Linux домена office.hitech2024.local.
17. На COD-CLI настройте SSO для обращения к `https://portal.hitech.ru` под пользователем hitech2024\LoganAdria и hitech2024\ByrdAidan (Доступ без ввода пароля, без ошибок и предупреждений у одного и запрет доступа у другого). Важно! Провека будет осуществляться в Mozilla Firefox.
